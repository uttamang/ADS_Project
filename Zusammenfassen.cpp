...
extern system_1 sys_pointer; // Globale Variable, die Input-, Output-, GND-Knoten enthalten
							 ...
							 *			"last_RLC".
							 *			Zun�chst wird die nxn Adjacenzmatrix mit "" initialisiert. n ist die Anzahl der gesamten
							 *			Knoten. Die Liste "last_RLC" wird von hinten an untersucht.
							 *			Es wird �berpr�ft, ob zwischen zwei Knoten schon Elemente vorhanden sind. Wenn es der
							 *			Fall ist, werden die beiden Elemente mit der Operation "||" verkn�pft.
							 * Argumente:
							 *
...
{
	unsigned char max_anzahl = k_index + 1;// weil nur Komma gez�hlt wird.
	int index = 1;
	...
	while (rt != NULL)
	{
		...
		if (Adjacenzmatrix[node_table[rt->NODE_1] - 1][node_table[rt->NODE_2] - 1] != "")// �berpr�ft, ob zwischen den beiden Knoten schon Elementname eingetragen wurde.
		{
...
/**************************************************************************************************
* Funktion: Die Funktion tr�gt einen Knotennamen in die Nodetabelle ein. Gleichzeitig wird ihm das
*			Index zugeordnet. Wenn eingetragen werden kann, wird true zur�ckgegeben.
*
* Argumente:
* string node : Knotename
* int index   : Zugeh�riges Index
*
* R�ckgabewert: true , wenn der Name eingetragen werden kann.
*/
...
/*********************************************************************************************************************************
* Funktion: Es wird Dreieck-Stern Transformation durchgef�hrt. Die Funktion pr�ft, ob es zwischen drei
*			Knoten eine Schleife existiert. Zun�chst wird zwischen zwei Knoten untersucht, wenn ein Element
...
*			true gesetzt.
*				Wenn ein Flag "dreieck_vorhanden" true gesetzt wird, wird ein weiteres Knote hinzugef�gt und als "st(i)" genannt.
*			Zur besseren �bersicht wird ein Legend erstellt. Die "Darstellung"-Objekt speichert den Nenner und Z�hler.
*			Sp�ter wird ihm eine Abk�rzung der Form "R{i,j}" zugeordnet. Zuletzt wird dann die ehemalige Elemente
*			zwischen drei Knoten gel�scht.
* kein Argument
* R�ckgabewert: true, wenn Dreieck-Stern Transformation m�glich ist.
********************************************************************************************************************************/
bool zusammenfassen::dreieck2stern()
{
	...
	{
		stern_position[0] = i;
		if (i == node_table[sys_pointer.GND] - 1) // wird �berspringt, falls das Knote ein GND ist.
			continue;
		for (char ii = 0; ii <= k_index && !dreieck_vorhanden; ii++)
		{
			if (ii == node_table[sys_pointer.GND] - 1)// wird �berspringt, falls das Knote ein GND ist.
				continue;
			if (Adjacenzmatrix[i][ii] != "") // Zwischen (1)(2) Eintrag vorhanden?
			{
				...
				{
					if (i3 == node_table[sys_pointer.GND] - 1)// wird �berspringt, falls das Knote ein GND ist.
						continue;
	...
	{
		for (char i = 0; i <= k_index; i++)// Adjacenzmatrix (x-dimension) um 1 vergr��ert.
		{
			Adjacenzmatrix[i].push_back("");
		}
		k_index++;
		//------------------------------------------------------//
		// Wegen Symmetrie, wird Adjacenzmatrix (y-dimension) um 1 vergr��ert.
		vector<string> init;
		init.assign(k_index + 1, "");
		Adjacenzmatrix.push_back(init);
		load_node_table("st" + to_string(k_index), k_index + 1);// Hier muss noch Index f�r st rein
