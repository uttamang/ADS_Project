...
//------------------------------------------------------------------------
/*********************************************************************************************************
* Funktion: Die Funktion f�hrt die lexikalische Analyse durch und charakterisiert das Ergebnis nach der
*			Analyse. Laut dem Input der Form "(Elementname):(Elementtype)(Knoten1,Knoten2)" ist der Z�hler
*			des Index
*			0 : Elementname
*			1 : Knoten1
*			2 : Knoten2
...
*			der Funktion wird der Zeiger auf das letzte Element zur�ckgegeben.
*
...
*			Aus diesem Grund wird am Anfang des Funktionsstart diese Information entnommen. Die zugeh�rigen
*			Z�hler-Index sind
*			0: Input-Knote
*			1: Output-Knote
*			2: GND-Knote
	...
	while ((tok = yylex()) != 0) {
		...
		if (tok == INTEGER1)
			...
			if (tok == IDENTIFIER && IP_LineNumber == 1)   // init datalist
			{
				...
				/*Die Beschreibung wird hier �berpr�ft.*/
				if (k_index <= 2)
				{
					if (k_index <= 1)
					{
